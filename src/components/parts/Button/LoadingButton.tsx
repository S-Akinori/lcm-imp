...
const LoadingButton = ({href='', className = '', loading, icon=<Spinner/>, done = false, doneChildren, color, style, target, children}:Props) => {
  ...
  return (
    <>
      {!href && <button className={btnClass} style={style}>{content}</button>}
